...
- [ ] 实现贾诩"毒士"：敌人获得负面状态时，层数 = 当前层数 + 新增层数（指数级翻倍）；获得"诡谋"层数；2层诡谋下回合减费。
...
1. 张角复活 `on_death`：
   ...
   ```
2. 贾诩的倍增：由于涉及对 StatusManager `apply_status` 前的干预，可能需要将贾诩的被动逻辑挂载在 `status_applied` 信号回调中，或作为一个修正过滤器。为避免死循环，倍增效果执行时应带上特殊的来源标签跳过自身再检测。
