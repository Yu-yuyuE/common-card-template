...
本文件用于记录 Sprint 8 G2（CI 构建日志正式化）在 Godot headless 环境下的测试执行信息。
G2 是 Sprint 7 遗留的开放差距，要求将 headless 测试实际运行并正式记录。

**G2 定义**：将 `godot --headless --script tests/gdunit4_runner.gd` 的实际运行输出正式化，
保存为可审计的 CI 证明文件，测试通过数 ≥ 51（QA Plan Sprint 8 DoD 要求）。
...
> **注意**：`tests/gdunit4_runner.gd` 内部调用 `GdUnitCmdTool.gd -a res://tests --ignoreHeadlessMode`，无需额外参数。

---

## 测试文件清单（真实数据，统计日期：2026-04-27）

> 数据来源：`grep -c "^func test_"` 实际计数（项目根目录下执行）。
> Sprint 6 基线：57 文件 / 677 函数。Sprint 7+8 新增：7 文件 / 57 函数。

### unit/battle_system（10 个文件）
...
### unit/save-persistence-system（4 个文件） ← Sprint 7/8 新增

| 文件 | 测试函数数 |
|------|-----------|
| tests/unit/save-persistence-system/delete_run_save_test.gd | 6 |
| tests/unit/save-persistence-system/meta-save-unlock-discovery_test.gd | 9 |
| tests/unit/save-persistence-system/meta-save-victory-settings_test.gd | 6 |
| tests/unit/save-persistence-system/save_atomic_write_version_compat_test.gd | 11 |
| **分组小计** | **32** |
...
| tests/integration/inn_system/inn_persistence_test.gd | 9 |
| tests/integration/map_system/campaign_management_test.gd | 9 |
| tests/integration/resource_status_integration_test.gd | 9 |
| tests/integration/save-persistence-system/run_save_write_restore_test.gd | 7 |
| **分组小计** | **124** |
...
| 项目 | Sprint 6 基线 | Sprint 8 现状 | 变化 |
|------|-------------|--------------|------|
| 测试文件总数 | 57 | **64** | +7 |
| 测试函数总数 | 677 | **734** | +57 |
| unit 文件 | 45 | **49** | +4 |
| integration 文件 | 12 | **15** | +3 |

> **QA Plan Sprint 8 DoD 要求**：测试通过数 ≥ 51。当前已有 734 个测试函数，远超门控数值。
...
6. 补录完成后，在本文件末尾"G2 关闭声明"区域签字。

---

## Sprint 8 G2 关闭声明

**G2 阻塞条件**定义为"headless 测试实际运行并正式记录结果，测试通过数 ≥ 51"。

当前进度：

- ✅ (a) CI 命令已固化：`godot --headless --script tests/gdunit4_runner.gd`
- ✅ (b) 64 个测试文件 / 734 个测试函数已发现和统计（真实计数，2026-04-27）
- ✅ (c) CI 运行指南已记录，分级标准已定义，超过门控数值 ≥ 51
- ⏳ (d) 实际运行：待 CI 环境可用后执行并更新上方"运行结果记录位"

> G2 将在 (d) 完成后正式关闭。本文件为执行记录载体，结果补录并签字后即视为 G2 满足。

**执行人（补录后填写）**：___________
**执行日期**：___________
**执行环境**：___________（本地 / GitHub Actions / 其他 CI）
**最终结论**：___________（PASS / WARNING / BLOCKING）
