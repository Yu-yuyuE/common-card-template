...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, WebSearch, WebFetch, new_task
---
...
- **Full spec**: `使用 setup-engine 技能 godot 4.6` — engine and version provided
- **Engine only**: `使用 setup-engine 技能 unity` — engine provided, version will be looked up
- **No args**: `使用 setup-engine 技能` — fully guided mode (engine recommendation + version)
...
- Read `design/gdd/game-concept.md` if it exists — extract genre, scope, platform
  targets, art style, team size, and any engine recommendation from `使用 brainstorm 技能`
- If no concept exists, inform the user:
  > "No game concept found. Consider running `使用 brainstorm 技能` first to discover what
...
## 4. Update COPILOT.md Technology Stack

Read `COPILOT.md` and update the Technology Stack section. Replace the
...
After updating COPILOT.md, create or update `.copilotcode/docs/technical-preferences.md` with
...
Run `使用 setup-engine 技能 refresh` to populate full reference docs at any time.
```
...
If invoked as `使用 setup-engine 技能 refresh`:
...
2. [If from 使用 brainstorm 技能] Run 使用 map-systems 技能 to decompose your concept into individual systems
3. [If from 使用 brainstorm 技能] Run 使用 design-system 技能 to author per-system GDDs (guided, section-by-section)
4. [If from 使用 brainstorm 技能] Run 使用 prototype 技能 [core-mechanic] to test the core loop
5. [If fresh start] Run 使用 brainstorm 技能 to discover your game concept
6. Create your first milestone: 使用 sprint-plan 技能 new
```
