...
> **Explicit invocation only**: This skill should only run when the user explicitly requests it with `使用 release-checklist 技能`. Do not auto-invoke based on context matching.
...
9. **Save the checklist** to
   `production/releases/[version].md`, creating the
