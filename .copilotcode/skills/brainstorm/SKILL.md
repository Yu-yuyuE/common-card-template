...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, WebSearch, ask_followup_question
---
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `ask_followup_question`** at key decision points throughout brainstorming:
   ...
   `ask_followup_question` to capture the decision with concise labels.
...
4. **Generate the game concept document** using the template at
   `.copilotcode/docs/templates/game-concept.md`. Fill in ALL sections from the
...
6. **Suggest next steps** (in this order — this is the professional studio
   ...
   - "Run `使用 setup-engine 技能 [engine] [version]` to configure the engine and populate version-aware reference docs"
   - "Use `使用 design-review 技能 design/gdd/game-concept.md` to validate completeness"
   - "Discuss vision with the `creative-director` agent for pillar refinement"
   - "Decompose the concept into individual systems with `使用 map-systems 技能` — maps dependencies, assigns priorities, and creates the systems index"
   - "Author per-system GDDs with `使用 design-system 技能` — guided, section-by-section GDD writing"
   - "Prototype the core loop with `使用 prototype 技能 [core-mechanic]`"
   - "Playtest the prototype with `使用 playtest-report 技能` to validate the hypothesis"
   - "If validated, plan the first sprint with `使用 sprint-plan 技能 new`"
