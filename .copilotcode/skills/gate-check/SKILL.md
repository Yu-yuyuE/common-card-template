...
**Distinct from `使用 project-stage-detect 技能`**: That skill is diagnostic ("where are we?").
...
- **With argument**: `使用 gate-check 技能 production` — validate readiness for that specific phase
- **No argument**: Auto-detect current stage using the same heuristics as
  `使用 project-stage-detect 技能`, then validate the NEXT phase transition
...
**Quality Checks:**
- [ ] Game concept has been reviewed (`使用 design-review 技能` verdict not MAJOR REVISION NEEDED)
...
**Required Artifacts:**
- [ ] Engine chosen (COPILOT.md Technology Stack is not `[CHOOSE]`)
- [ ] Technical preferences configured (`.copilotcode/docs/technical-preferences.md` populated)
...
- [ ] At least 1 playtest report (or `使用 playtest-report 技能` has been run)
...
- [ ] QA test plan exists
- [ ] Balance data has been reviewed (`使用 balance-check 技能` run)
- [ ] Release checklist completed (`使用 release-checklist 技能` or `使用 launch-checklist 技能` run)
...
- For performance checks: `Read` technical-preferences.md and compare against any
  profiling data in `tests/performance/` or recent `使用 perf-profile 技能` output
...
- "Performance profiling data isn't available. Would you like to run `使用 perf-profile 技能`?"
...
### Blockers
1. **No Architecture Decision Records** — Run `使用 architecture-decision 技能` to create one
...
- **No game concept?** → `使用 brainstorm 技能` to create one
- **No systems index?** → `使用 map-systems 技能` to decompose the concept into systems
- **Missing design docs?** → `使用 reverse-document 技能` or delegate to `game-designer`
- **Missing ADRs?** → `使用 architecture-decision 技能`
- **Tests failing?** → delegate to `lead-programmer` or `qa-tester`
- **No playtest data?** → `使用 playtest-report 技能`
- **Performance unknown?** → `使用 perf-profile 技能`
- **Not localized?** → `使用 localize 技能`
- **Ready for release?** → `使用 launch-checklist 技能`
