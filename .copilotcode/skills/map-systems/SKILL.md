...
argument-hint: "[optional: 'next' to pick highest-priority undesigned system, or a system name to hand off to 使用 design-system 技能]"
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, ask_followup_question, TodoWrite
---
...
- **No argument**: `使用 map-systems 技能` — Run the full decomposition workflow (Phases 1-5)
  to create or update the systems index.
- **`next`**: `使用 map-systems 技能 next` — Pick the highest-priority undesigned system
  from the index and hand off to `使用 design-system 技能` (Phase 6).
...
- Read `design/gdd/game-concept.md` — **fail with a clear message if missing**:
  > "No game concept found at `design/gdd/game-concept.md`. Run `使用 brainstorm 技能` first
...
Then use `ask_followup_question` to capture feedback:
...
Use `ask_followup_question` to ask: "Does this dependency ordering look right? Any
...
Use `ask_followup_question` to ask: "Do these priority assignments match your vision?
...
Using the template at `.copilotcode/docs/templates/systems-index.md`, populate the
...
## 7. Phase 6: Design Individual Systems (Handoff to 使用 design-system 技能)

This phase is entered when:
- The user says "yes" to designing systems after creating the index
- The user invokes `使用 map-systems 技能 [system-name]`
- The user invokes `使用 map-systems 技能 next`
...
Use `ask_followup_question` for: "Start designing [system-name] now, pick a different
system, or stop here?"

### Step 6b: Hand Off to 使用 design-system 技能

Once a system is selected, invoke the `使用 design-system 技能 [system-name]` skill.

The `使用 design-system 技能` skill handles the full GDD authoring process:
...
- Runs `使用 design-review 技能` when complete
- Updates the systems index

**Do not duplicate the 使用 design-system 技能 workflow here.** This skill owns the systems
*index*; `使用 design-system 技能` owns individual system *GDDs*.

### Step 6c: Loop or Stop

After `使用 design-system 技能` completes, use `ask_followup_question`:
...
- "Run `使用 design-system 技能 [system-name]` to write the next system's GDD"
- "Run `使用 design-review 技能 [path]` on each completed GDD to validate quality"
- "Run `使用 gate-check 技能 pre-production` to check if you're ready to start building"
- "Prototype the highest-risk system with `使用 prototype 技能 [system]`"
- "Plan the first implementation sprint with `使用 sprint-plan 技能 new`"
...
2. **ask_followup_question** at every decision point (Explain -> Capture pattern):
   ...
   - Phase 6: "Start designing, pick different, or stop?" then hand off to `使用 design-system 技能`
...
5. **Handoff**: Individual GDD authoring is owned by `使用 design-system 技能`, which handles
