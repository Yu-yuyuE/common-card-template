...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, new_task, ask_followup_question, TodoWrite
---
...
> "Usage: `使用 design-system 技能 <system-name>` — e.g., `使用 design-system 技能 combat-system`
> Run `使用 map-systems 技能` first to create the systems index, then use this skill
...
- **Game concept**: Read `design/gdd/game-concept.md` — fail if missing:
  > "No game concept found. Run `使用 brainstorm 技能` first."
- **Systems index**: Read `design/gdd/systems-index.md` — fail if missing:
  > "No systems index found. Run `使用 map-systems 技能` first to map your systems."
...
Use the template structure from `.copilotcode/docs/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `ask_followup_question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `ask_followup_question` to capture the decision.
...
**Agent delegation**: For complex mechanics, use the `new_task` tool to delegate to
...
delegate to `systems-designer` via the `new_task` tool. Provide:
...
Use `ask_followup_question`:
- "Run `使用 design-review 技能` now to validate the GDD?"
...
- "What's next?"
  - Options:
    ...
    - "Run `使用 gate-check 技能`" — if enough MVP systems are designed
...
**When delegating via `new_task` tool**:
...
- The main session presents the agent's output to the user via `ask_followup_question`
