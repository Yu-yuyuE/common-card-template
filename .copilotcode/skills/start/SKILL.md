...
user-invocable: true
allowed-tools: Read, Glob, Grep, ask_followup_question
---
...
Check:
- **Engine configured?** Read `.copilotcode/docs/technical-preferences.md`. If the
...
> **Welcome to Copilot Code Game Studios!**
>
...
2. Briefly explain what `使用 brainstorm 技能` does (guided ideation using professional
   frameworks — MDA, player psychology, verb-first design)
3. Recommend running `使用 brainstorm 技能 open` as the next step
4. Show the recommended path:
   - `使用 brainstorm 技能` — discover your game concept
   - `使用 setup-engine 技能` — configure the engine (brainstorm will recommend one)
   - `使用 map-systems 技能` — decompose the concept into systems and plan GDD writing order
   - `使用 prototype 技能` — test the core mechanic
   - `使用 sprint-plan 技能` — plan the first sprint
...
3. Recommend running `使用 brainstorm 技能 [their hint]` to develop it
4. Show the recommended path:
   - `使用 brainstorm 技能 [hint]` — develop the idea into a full concept
   - `使用 setup-engine 技能` — configure the engine
   - `使用 map-systems 技能` — decompose the concept into systems and plan GDD writing order
   - `使用 prototype 技能` — test the core mechanic
   - `使用 sprint-plan 技能` — plan the first sprint
...
2. Based on their answers, offer two paths:
   - **Formalize first**: Run `使用 brainstorm 技能` to structure the concept into a
   ...
   - **Jump to engine setup**: If they're confident in their concept, go
     straight to `使用 setup-engine 技能` and write the GDD manually afterward
3. Show the recommended path (adapted to their choice):
   - `使用 brainstorm 技能` or `使用 setup-engine 技能` (their pick)
   - `使用 design-review 技能` — validate the concept doc
   - `使用 map-systems 技能` — decompose the concept into individual systems with dependencies and priorities
   - `使用 design-system 技能` — author per-system GDDs (guided, section-by-section)
   - `使用 architecture-decision 技能` — make first technical decisions
   - `使用 sprint-plan 技能` — plan the first sprint
...
2. Recommend running `使用 project-stage-detect 技能` for a full analysis
3. If the engine isn't configured, note that `使用 setup-engine 技能` should come first
4. Show the recommended path:
   - `使用 project-stage-detect 技能` — full gap analysis
   - `使用 setup-engine 技能` — if not configured
   - `使用 design-system 技能` — if systems index exists but GDDs are incomplete
   - `使用 gate-check 技能` — validate readiness for next phase
   - `使用 sprint-plan 技能` — organize the work
...
or offer to run it for them. Either way, the `使用 start 技能` skill's job is done
...
- **User is returning (engine configured, concept exists)**: Skip onboarding
  ...
  you left off? Try `使用 sprint-plan 技能` or just tell me what you'd like to work on."
