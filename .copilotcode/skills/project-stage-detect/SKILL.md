...
if it exists, use its value (explicit override from `使用 gate-check 技能`). Otherwise,
...
| **Polish** | Explicit only (set by `使用 gate-check 技能` Production → Polish gate) |
| **Release** | Explicit only (set by `使用 gate-check 技能` Polish → Release gate) |
...
- "I found a game concept but no systems index. Have you decomposed the concept into individual systems yet, or should we run `使用 map-systems 技能`?"
...
Use template: `.copilotcode/docs/templates/project-stage-report.md`
...
If user provided a role argument (e.g., `使用 project-stage-detect 技能 programmer`):
...
使用 project-stage-detect 技能

# Programmer-focused analysis
使用 project-stage-detect 技能 programmer

# Designer-focused analysis
使用 project-stage-detect 技能 designer
```
...
- **Concept exists but no systems index?** → `使用 map-systems 技能` to decompose into systems
- **Missing design docs?** → `使用 reverse-document 技能 design src/[system]`
- **Missing architecture docs?** → `使用 architecture-decision 技能` or `使用 reverse-document 技能 architecture`
- **Prototypes need documentation?** → `使用 reverse-document 技能 concept prototypes/[name]`
- **No sprint plan?** → `使用 sprint-plan 技能`
- **Approaching milestone?** → `使用 milestone-review 技能`
